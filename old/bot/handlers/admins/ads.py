...
from old.bot.handlers.admins.sender import admin_send_message_all, send_message_admin
from old.bot.utils import IsAdmin, AdsStates
from old.core.config import bot

router = Router()
from old.bot.handlers.admins.keyboards import initial_options, media_type_options
...
async def ask_buttons(message: Message, state: FSMContext):
    await message.answer(
        "<b>Post uchun tugmalar yaratishni hohlaysizmi?</b>",
        reply_markup=initial_options(),
    )
...
async def button_url(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Tugma qo'shildi! Keyingi vazifa?", reply_markup=initial_options()
    )
    await state.set_state(AdsStates.confirm_buttons)
